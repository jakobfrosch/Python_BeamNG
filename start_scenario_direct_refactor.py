...
file_handler = logging.FileHandler("logs/XBeamNG_Simulation.log")
file_handler.setLevel(logging.INFO)
logging.basicConfig(level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s',
                    handlers=[
                        logging.FileHandler("logs/BeamNG_Simulation.log"),  # Log-Datei
                        logging.StreamHandler(sys.stdout)       # Konsolenausgabe
                    ])
...
def run_monitor_in_thread(vehicles, colission_stop_cond, road_polygons):
    # asyncio.run() führt die Coroutine in einer Event-Loop aus
    asyncio.run(monitor_vehicles_periodically(vehicles, colission_stop_cond, road_polygons))
...
def monitor_vehicle_position(vehicle, colission_stop_cond, combined_polygon):
    with sensor_lock:
...
async def monitor_vehicles_periodically(vehicles, colission_stop_cond, combined_polygon):
    #combined_polygon = unary_union([polygon for polygon in road_polygons if polygon.is_valid])
    while process_stopped is False:
        for vehicle in vehicles:
            monitor_vehicle_position(vehicle, colission_stop_cond, combined_polygon)
...
def run_async_tasks(loop, vehicles, colission_stop_cond, road_polygons):
    asyncio.set_event_loop(loop)
    loop.run_until_complete(monitor_vehicles_periodically(vehicles, colission_stop_cond, road_polygons))
    loop.close()
def terminate_software(bng):
    logging.info("Die Software wird beendet.")
    thread.join()
    bng.close()
    print("BNG closed")
    #sys.exit(0)  # Beendet das Programm
async def check_time(target_time, bng):
    while True:
...
def run_async_task_time(loop, t, bng):
    asyncio.set_event_loop(loop)
    target_time = time.time() + t
    loop.run_until_complete(check_time(target_time, bng))
    loop.close()
argumentsmanual = ['FollowLeadingVehicle5_time.xosc', 'FollowLeadingVehicle4_time.xosc']
path = 'C:\\Users\\stefan\\Documents\\'
for argument in argumentsmanual:
    out_of_road_list = []
    waypointLines = []
    off_the_road_vehicles = []
    vehicles = []
    ...
    if time_stop_cond:
        ...
        thread = threading.Thread(target=run_async_task_time, args=(loop, t, bng))
        thread.start()
    loop = asyncio.new_event_loop()
    thread = threading.Thread(target=run_async_tasks, args=(loop, vehicles, colission_stop_cond, road_polygons))
    thread.start()
    if start_condition_finished:
        logging.info("Startbedingungen nicht definiert. Springe gleich zu Events.")
    else :
        print(f'conditiontype: {start_condition.condition_type}')
